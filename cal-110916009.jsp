...
<!-- (1)參考資料:上學期許揚老師的期末作業 也是計算機的實作 網路上遇到什麼問題就查什麼 主要是找語法 沒有特別抄太多程式碼 另外也有詢問之前上過課的學長
     (2)操作說明:正常計算機的操作
                一次只能做兩個運算元的計算 支持加減乘除、負數、小數點、次方、根號、10的次方 先前計算的紀錄會留在上方 可以參考並繼續運算
                注意若按>2個運算元 最剛開始的運算元會被取代
     (3)自評:95 理由:這樣的計算機雖然能做運算 但沒自信自評100 畢竟一次只能兩個數字計算
     (4)參考網路上html計算機的架構並用上學期課程學到的css改了計算機的整體設計
   ...
   <body>
      ...
      <%!
         ...
         private void displayFloat(String formula)
         {
            ...
            for (int i = 0; i < floatHow; i++)
               y = y*0.1;
            floatHow++;
            answer = Math.round((answer + y) * 100000000.0) / 100000000.0;
         }
      %>
      <!-- 計算方法 -->
      <%
         if(request.getParameter("no") != null)
         {
            ...
            try
            {
               ...
               else if(request.getParameter("no").equals("=")) //按=的功能
               {
                  ...
                  switch(operator){
                     case 1: 
                        answer = Math.round((num1 + num2) * 100000000.0) / 100000000.0; break;
                     case 2: 
                        answer = Math.round((num1 - num2) * 100000000.0) / 100000000.0; break;
                     case 3: 
                        answer = Math.round((num1 * num2) * 100000000.0) / 100000000.0; break;
                     case 4: 
                        answer = Math.round((num1 / num2) * 100000000.0) / 100000000.0; break;
                     case 5:
                        answer = Math.round(Math.pow(num1, num2) * 100000000.0) / 100000000.0; break;
                     case 6:
                        answer = Math.round(Math.pow(num1, (1/num2)) * 100000000.0) / 100000000.0; break;
                     case 7:
                        answer = Math.round((num1 * (Math.pow(10, num2))) * 100000000.0) / 100000000.0; break;
                     default:
                  }
               }
      ...
      <form action="cal-110916009.jsp" method="post" name="Calculator">        
         <table border="15">
            ...
            <tr>
               <td style="border:3pt solid #7B7B7B;" align='right' colspan="5">
                  <font face="Microsoft JhengHei" size="6">
                     <%= answer %>
                  </font>
               </td>
            </tr>
